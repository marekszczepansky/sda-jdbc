...
public class EmpDAOJdbcImpl implements EmpDAO {
    ...
    public void create(Employee employee) throws SQLException {
        try (Connection conn = jdbcConnectionManager.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                    "insert into Emp " +
                            "(empno, ename, job, manager, hiredate, salary, commision, deptno) " +
                            "values(?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void update(Employee employee) throws SQLException {
        try (Connection conn = jdbcConnectionManager.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                    "update emp " +
                            "set ename = ?, job = ?, manager = ?, hiredate = ?, salary = ?, commision = ?, deptno = ? " +
                            "where empno = ?");
    ...
    public BigDecimal getTotalSalaryByDept(int dept) throws Exception {
        try (Connection conn = jdbcConnectionManager.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                    "select sum(salary) as totalSalary from emp where deptno = ?"
            );
            ps.setInt(1, dept);

            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getBigDecimal("totalSalary");
            }
        }
        return null;
    }
